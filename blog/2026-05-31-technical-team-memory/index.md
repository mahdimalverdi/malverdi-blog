...
description: چرا مستند فنی اگر جای درست، مسئول روشن و فرایند نگهداشت نداشته باشد، به‌جای کمک به تیم می‌تواند گمراه‌کننده شود.
---
...
مدعای این متن ساده است: حافظه‌ی فنی تیم وقتی قابل اعتماد است که کنار جایی باشد که تغییر از آن‌جا شروع می‌شود، مسئول روشن داشته باشد، و برای مخاطب مشخص نوشته شود. اگر یکی از این سه شرط نباشد، مستند به‌جای اینکه فهم سیستم را آسان کند، خودش به منبع ابهام تبدیل می‌شود.
...
در سازمان‌های بزرگ، این وضعیت معمولاً به چند نسخه از یک واقعیت ختم می‌شود: چند راهنما برای یک کار، چند توضیح برای یک رفتار، و چند صفحه که معلوم نیست کدام هنوز معتبر است. راه‌حل این نیست که سند بیشتری بنویسیم؛ باید یکی مرجع معتبر باشد و بقیه حذف، ادغام یا صریحاً قدیمی اعلام شوند.

:::danger[خطر]
سندی که بیرون از روند تغییر می‌ماند، کم‌کم از حافظه‌ی تیم به بایگانی حدس‌ها تبدیل می‌شود.
:::
...
برای هر سند فنی مهم باید بتوانیم به چند پرسش ساده جواب بدهیم: این سند کجا مرجع معتبر دارد؟ چه کسی مسئول آن است؟ آخرین بار کی و همراه با چه تغییری به‌روز شده؟ اگر غلط بود، از چه مسیری اصلاح می‌شود؟ اگر دیگر معتبر نیست، آیا این را صریح گفته‌ایم؟
...
هدف مستند، زیاد کردن متن نیست؛ ساختن حافظه‌ای است که تیم بتواند به آن اعتماد کند. حافظه‌ای که فقط در ذهن آدم‌ها باشد، با رفتن آدم‌ها کم‌رنگ می‌شود. حافظه‌ای که فقط در ویکی دور از کد باشد، با تغییر سیستم عقب می‌افتد. حافظه‌ای که کنار کد، داخل مخزن، کنار تصمیم و در روند تغییر باشد، شانس زنده ماندن دارد.
...
حافظه‌ی فنی زنده با متن زیاد ساخته نمی‌شود؛ با مرجع معتبر، مسئول روشن، و نگهداشت پیوسته ساخته می‌شود. پس مسئله این نیست که مستند داشته باشیم یا نه. مسئله این است که مستندمان بخشی از کار مهندسی باشد، نه کاری تزئینی پس از آن. اگر مستند کنار روند واقعی تغییر بماند، به حافظه‌ی فنی تیم تبدیل می‌شود؛ اگر جدا بماند، دیر یا زود فقط بایگانی محترمانه‌ای از حدس‌ها خواهد بود.
