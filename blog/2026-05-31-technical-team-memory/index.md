...
این نگاه در کتاب *Software Engineering at Google* هم دیده می‌شود: مستند خوب محصول را شکل می‌دهد. نوشتن قرارداد API ابهام‌های طراحی را زودتر آشکار می‌کند، راهنمای استفاده مسیرهای گیج‌کننده را نشان می‌دهد، و جواب دادن به پرسش‌های تکراری، زمان تیم را از توضیح‌های دوباره آزاد می‌کند.
...
کتاب همین دام را توضیح می‌دهد: هزینه‌ی نوشتن امروز پرداخت می‌شود، اما سودش بعداً و برای آدمی دیگر دیده می‌شود. طبیعی است که در فشار کار عقب بیفتد؛ مگر اینکه تیم آن را بخشی از تعریف کار تمام‌شده بداند.
...
کتاب همین مسیر را با نمونه‌ی GooWiki توضیح می‌دهد. GooWiki اول راه‌حل ساده‌ای برای اشتراک دانش بود، اما با رشد سازمان، نداشتن مالک روشن و فرایند مشخص باعث شد سندها قدیمی، تکراری و پراکنده شوند. حتی برای یک کار مشخص مثل راه‌اندازی Borg چند سند مختلف وجود داشت که فقط بعضی از آن‌ها نگه داشته شده بودند.
...
کتاب همچنین بین دو نوع خواننده فرق می‌گذارد. گاهی خواننده با پرسشی مشخص سراغ مستند می‌آید؛ او «جست‌وجوگر» است و به جواب سریع، دقیق و قابل اسکن نیاز دارد. گاهی هم خواننده تازه با سیستم روبه‌رو شده و هنوز نمی‌داند دنبال چه چیزی بگردد؛ او «رهگذر» است و به صفحه‌ی ورودی، مسیر یادگیری و توضیح مفهومی نیاز دارد.
...
همان‌طور که کد بدون review قابل اعتماد نیست، مستند مهم هم بدون review قابل اعتماد نمی‌ماند. اما review مستند فقط غلط‌گیری نگارشی نیست. کتاب سه زاویه‌ی مهم را از هم جدا می‌کند: دقت فنی، وضوح برای مخاطب، و کیفیت نوشتار.
