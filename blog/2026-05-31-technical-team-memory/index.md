...
این نوشته درباره‌ی همین حافظه‌ی فنی است: چه چیزهایی را کد می‌گوید، چه چیزهایی را باید مستند بگوید، و این حافظه کجا باید زندگی کند تا با تغییر سیستم زنده بماند.
...
## مستند برای نمایش نیست؛ برای کم‌کردن هزینه‌ی فهم است
...
اگر مستندها بدند، دلیلش فقط این نیست که مهندس‌ها نمی‌خواهند بنویسند. خیلی وقت‌ها خود محیط نوشتن بد طراحی شده است: ابزار از کد جداست، تغییر سند در بازبینی دیده نمی‌شود، صفحه‌ها مالک ندارند، نمونه‌ها قدیمی می‌شوند، و نوشتن مستند مثل کاری اضافه پس از پایان مهندسی دیده می‌شود.
...
مشکل از وقتی شروع می‌شود که همان صفحه‌ها تبدیل می‌شوند به تنها جای حقیقت فنی. کد در مخزن تغییر می‌کند، تنظیمات در محیط اجرا عوض می‌شوند، API شکل تازه‌ای می‌گیرد، اما سند بیرون از این چرخه می‌ماند. در سازمان‌های بزرگ، این وضعیت معمولاً به سندهای قدیمی، تکراری و پراکنده ختم می‌شود: برای یک کار مشخص چند راهنما پیدا می‌شود، اما معلوم نیست کدام هنوز معتبر است.
...
وقتی می‌گوییم «مستند»، از یک چیز واحد حرف نمی‌زنیم. بخشی از حافظه‌ی فنی در کامنت تابع و قرارداد API می‌ماند، بخشی در README و راهنمای راه‌اندازی، بخشی در سند طراحی، و بخشی در صفحه‌ی ورودی یا ویکی. مشکل از جایی شروع می‌شود که همه‌ی این‌ها را در یک ظرف بریزیم و انتظار داشته باشیم برای همه‌ی خواننده‌ها خوب کار کند.
...
از یک زاویه‌ی دیگر، دو نوع خواننده داریم. گاهی خواننده با پرسشی مشخص سراغ مستند می‌آید؛ او «جست‌وجوگر» است و به جواب سریع، دقیق و قابل اسکن نیاز دارد. گاهی هم خواننده تازه با سیستم روبه‌رو شده و هنوز نمی‌داند دنبال چه چیزی بگردد؛ او «رهگذر» است و به صفحه‌ی ورودی، مسیر یادگیری و توضیح مفهومی نیاز دارد.

:::note[پرسش قبل از نوشتن]
این سند قرار است تصمیم چه کسی را ساده‌تر کند؟ اگر جواب روشن نیست، احتمالاً خود سند هم روشن نخواهد شد.
:::

همین تفاوت مخاطب، نوع مستند را هم تعیین می‌کند. مرجع، آموزش، سند طراحی، توضیح مفهومی و صفحه‌ی ورودی هرکدام نقش جدا دارند. اگر سند مرجع را مثل آموزش بنویسیم، پیدا کردن پاسخ سخت می‌شود. اگر سند طراحی را مثل راهنمای نصب بنویسیم، چرایی تصمیم‌ها گم می‌شود.
...
سند طراحی جای توضیح «چرا» است. باید روشن کند مسئله چیست، هدف و غیرهدف چیست، راه‌حل پیشنهادی چیست، چه گزینه‌هایی رد شده‌اند، بده‌بستان‌ها کجا هستند، چه نگرانی‌هایی درباره‌ی امنیت، حریم خصوصی، داده، عملیات یا مهاجرت وجود دارد، و تغییر چطور مرحله‌به‌مرحله انجام می‌شود.

آموزش گام‌به‌گام کارش چیز دیگری است. باید خواننده را از نقطه‌ی شروع به یک نتیجه‌ی واقعی برساند: اجرای سرویس، ساخت اولین درخواست، نوشتن اولین تست، یا دیدن اولین خروجی درست. آموزش خوب پیش‌نیازها را روشن می‌کند، قدم‌ها را شماره‌گذاری می‌کند و به خواننده می‌گوید در هر مرحله باید چه چیزی ببیند.

کامنت خوب هم متن اضافه نیست؛ نزدیک‌ترین بخش حافظه‌ی فنی به کد است. کامنت سطح فایل باید بگوید این فایل چه نقشی در سیستم دارد و خواننده چرا باید از این‌جا شروع کند. کامنت کلاس باید مسئولیت و رابطه‌ی آن با بقیه‌ی اجزا را روشن کند. کامنت تابع باید قرارداد رفتار را بگوید، نه فقط نام تابع را تکرار کند.
...
اینجا باید بین سه چیز فرق بگذاریم. **کنار کد** یعنی کامنت تابع، توضیح رفتار حساس و قرارداد نزدیک به پیاده‌سازی. **داخل مخزن** یعنی README، پوشه‌ی docs، راه‌اندازی، تست، ساخت، انتشار و سند طراحی نسخه‌دار. **ویکی یا Confluence** یعنی دانش بین‌تیمی، سیاست‌ها، گزارش‌ها و مسیرهای ورودی که کمتر با تغییر مستقیم کد عوض می‌شوند.
